...
pub(crate) fn client_handshake(
    ...
    let challenge_hex = challenge_line
        .strip_prefix("TSU4C ")
        .ok_or("receiver did not offer a TSU4 challenge")?;
    let challenge = decode_array::<NONCE_LEN>(challenge_hex)?;
    let hello_mac = hmac_sha256(
        &key.0,
        &joined(b"tsu4 client hello\0", &[&challenge, hello.as_bytes()]),
    );
    writeln!(stream, "TSU4 {hello} {}", hex(&hello_mac))?;
    stream.flush()?;

    let server_line = read_line_limited(stream, MAX_HANDSHAKE_LINE)?;
    let fields: Vec<_> = server_line.split_whitespace().collect();
    if fields.len() != 3 || fields[0] != "TSU4S" {
        return Err("invalid TSU4 server response".into());
    }
    ...
    let expected = hmac_sha256(&key.0, &joined(b"tsu4 server hello\0", &[&transcript]));
    if !constant_time_eq(&supplied, &expected) {
        return Err("receiver authentication failed".into());
    }
    let finish = hmac_sha256(&key.0, &joined(b"tsu4 client finish\0", &[&transcript]));
    writeln!(stream, "TSU4F {}", hex(&finish))?;
    stream.flush()?;
...
pub(crate) fn server_handshake(
    ...
    let response = hmac_sha256(&key.0, &joined(b"tsu4 server hello\0", &[&transcript]));
    writeln!(stream, "TSU4S {} {}", hex(&server_nonce), hex(&response))?;
    stream.flush()?;
    ...
    let expected_finish = hmac_sha256(&key.0, &joined(b"tsu4 client finish\0", &[&transcript]));
...
pub(crate) fn lane_mac(key: &[u8; 32], session: &[u8; 16], lane: usize) -> [u8; 32] {
    hmac_sha256(
        key,
        &joined(b"tsu4 lane\0", &[session, &(lane as u64).to_be_bytes()]),
    )
}
...
pub(crate) fn udp_tag_parts(key: &[u8; 32], header: &[u8], payload: &[u8]) -> [u8; UDP_TAG_LEN] {
    let full = hmac_sha256_parts(key, &[b"tsu4 udp\0", header, payload]);
...
fn derive_session(psk: &[u8; 32], transcript: &[u8]) -> SessionAuth {
    let root = hmac_sha256(psk, &joined(b"tsu4 session root\0", &[transcript]));
