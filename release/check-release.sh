...
package="target/package/tsunami-udp-$version.crate"
if [[ ! -f $package ]]; then
...
temporary=$(mktemp -d "${TMPDIR:-/tmp}/tsunami-udp-release.XXXXXX")
trap 'rm -rf "$temporary"' EXIT
tar -xzf "$package" -C "$temporary"
cargo install --locked --path "$temporary/$prefix" --root "$temporary/install"

binary="$temporary/install/bin/tsunami-udp"
actual_version=$($binary --version)
if [[ $actual_version != "tsunami-udp $version" ]]; then
