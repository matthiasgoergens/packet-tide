...
if [[ $# -lt 2 || $# -gt 3 ]]; then
  echo "usage: $0 MATRIX_FILE OUTPUT_DIR [RANDOMIZATION_SEED]" >&2
  exit 2
fi
...
python3 "$ROOT/lab/randomize-matrix.py" \
  "$MATRIX_FILE" "$RANDOMIZATION_SEED" >"$DESIGN"
...
while IFS=$'\t' read -r BLOCK_ORDER BLOCK_ID FILE_BYTES RATE_MBIT RTT_MS LOSS_PERCENT SEED TRANSPORT_ORDER; do
  ...
  for TRANSPORT in "${TRANSPORTS[@]}"; do
    ...
    cp "$WORK_RESULTS/result-$TRANSPORT-$FILE_BYTES-$RATE_MBIT-$RTT_MS-$LOSS_PERCENT-$SEED.json" "$OUTPUT_DIR/"
  done
  python3 "$ROOT/lab/evaluate-block.py" "$OUTPUT_DIR" "$BLOCK_ID"
done <"$DESIGN"
